...
class BrowserModPopup extends LitElement {
    ...
    openDialog() {
        ...
        customElements.whenDefined("card-mod").then(() => {
            (customElements.get("card-mod") as any)?.applyToElement?.(this, "more-info", this.card_mod?.style ?? "");
        });
    ...
    render() {
        ...
        return html`
            ...
            >
                ${this.timeout ? html` <div slot="heading" class="progress"></div> ` : ""}
                ${this.title
                    ? html`
...
if (!customElements.get("browser-mod-popup")) customElements.define("browser-mod-popup", BrowserModPopup);
    ...
    return class PopupMixinClass extends SuperClass {
        ...
        constructor() {
            ...
            this._popupEl.addEventListener("hass-action", async (ev: CustomEvent) => {
                if (
                    (ev.detail.action === "tap" && CLOSE_POPUP_ACTIONS.has(ev.detail.config?.tap_action?.action)) ||
                    (ev.detail.action === "hold" && CLOSE_POPUP_ACTIONS.has(ev.detail.config?.hold_action?.action)) ||
                    (ev.detail.action === "double_tap" &&
                        CLOSE_POPUP_ACTIONS.has(ev.detail.config?.double_tap_action?.action))
                ) {
                    this._popupEl.do_close();
                }
                ev.stopPropagation();
            ...
            const historyListener = async (ev) => {
                ...
                if (popupState) {
                    if (!popupState.open) {
                        if (this._popupEl?.open && this._popupEl?.dismissable) this._popupEl.do_close();
                    }
                }
            };
        ...
        async showMoreInfo(entityId, large = false, ignore_popup_card = undefined) {
            ...
            if (large) {
                ...
                const dialog: any = base.shadowRoot.querySelector("ha-more-info-dialog");
